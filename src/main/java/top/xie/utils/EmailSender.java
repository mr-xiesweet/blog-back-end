...
public class EmailSender {
    ...
    private EmailSender() {
        EmailSender.config(EmailSender.SMTP_QQ(false), "2936161670@qq.com", "llzguyikdmdldeia");
    }
    ...
    public static void sendRegisterVerifyCode(String code,String address) throws Exception{
        EmailSender.subject("阳光沙滩注册验证码")
                .from("阳光沙滩")
                .text("您的验证码是："+code+"  ,有效期为5分钟,如非本人操作，请忽略此邮件")
                .to(address)
                .send();
    }

    public static void sendCommentNotify(String notifyContent,String address) throws Exception{
        EmailSender.subject("阳光沙滩-评论通知")
                .from("阳光沙滩")
                .text(notifyContent)
                .to(address)
                .send();
